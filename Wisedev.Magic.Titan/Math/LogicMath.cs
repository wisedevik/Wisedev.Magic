...
public unsafe class LogicMath
{
    ...
    public static int GetAngle(int x, int y)
    {
        ...
        if (x <= 0 && y > 0)
        {
            if (num < y)
            {
                return 90 + ATAN_TABLE[(num << 7) / y];
            }

            return 180 - ATAN_TABLE[(y << 7) / num];
        }
        ...
        if (x < 0 && y <= 0)
        {
            if (num2 >= num)
            {
                ...
                return 270 - ATAN_TABLE[(num << 7) / num2];
            }

            return 180 + ATAN_TABLE[(num2 << 7) / num];
        }
        ...
        return NormalizeAngle360(360 - ATAN_TABLE[(num2 << 7) / num]);
    }
    ...
    {
        fixed (byte* sqrtTable = SQRT_TABLE)
        {
            if (value >= 0x10000)
            {
                int num;
